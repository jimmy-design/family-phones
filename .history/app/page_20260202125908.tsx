          ...
          {!activeFeature && (
            <>
              ...
              >
                ...
                <div className="flex flex-row gap-4 py-2 justify-between items-stretch">
                  {(statistics.length > 0 ? statistics : (current.stats || [])).map((stat, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-4 rounded-2xl text-center flex-1 min-w-0">
                        <div className="text-2xl sm:text-3xl md:text-2xl lg:text-3xl font-extrabold truncate leading-tight">
                          {typeof displayValue === 'number' ? `KES ${displayValue.toLocaleString('en-KE')}` : displayValue}
                        </div>
                        <div className="text-sm opacity-90 mt-1 truncate">{stat.label}</div>
                      </div>
                    );
                  })}
                </div>
