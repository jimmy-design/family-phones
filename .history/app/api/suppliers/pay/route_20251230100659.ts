...
export async function POST(request: Request) {
  try {
    ...
    const { data: existing, error: fetchErr } = await supabase
      .from('suppliers')
      .select('paid, total_owed, phone, phone_number, name, refno')
      .eq('id', id)
      .limit(1)
      .single();
    ...
    try {
      ...
      if (phone) {
        ...
        const message = `Hello ${supplierName || supplierRef || 'Supplier'}, we received KES ${amt.toFixed(2)}. Ref: ${supplierRef || '—'}. Remaining: KES ${newBalance.toFixed(2)}.`;
