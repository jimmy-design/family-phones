          ...
          {!activeFeature && (
            <>
              ...
              >
                ...
                <div className="flex flex-row md:grid md:grid-cols-3 gap-4 py-2 overflow-x-auto">
                  {(statistics.length > 0 ? statistics : (current.stats || [])).map((stat, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-4 rounded-2xl text-center flex-none min-w-[140px] sm:min-w-[160px] md:flex-1 md:basis-0 overflow-hidden">
                        <div className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold truncate leading-tight">
                          {typeof displayValue === 'number' ? `KES ${displayValue.toLocaleString('en-KE')}` : displayValue}
                        </div>
                        <div className="text-xs sm:text-sm opacity-90 mt-1 truncate">{stat.label}</div>
                      </div>
                    );
                  })}
                </div>
