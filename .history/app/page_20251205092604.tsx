...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        let endpoint = "";
        if (activeFeature === "Customers") endpoint = "/api/customers";
        else if (activeFeature === "Inventory") endpoint = "/api/inventory";
        else if (
        ...
        // ----------------------------------------------------------------
        // ** ADDED: Logic for Recorded Payments (Outstanding Invoices) **
        else if (activeFeature === "Recorded Payments")
          endpoint = "/api/invoices/outstanding"; // Calls the new dedicated route
        // ----------------------------------------------------------------
        else
          endpoint = `/api/feature/${activeFeature
            .toLowerCase()
            .replace(/\s/g, "")}`;
  ...
  const getOrderedKeys = (row: any) => {
    ...
    if (activeFeature?.toLowerCase().includes("invoice") || activeFeature?.toLowerCase().includes("quotation")) {
      const excludedCols = ["created_by", "createdby", "created_at", "createdat", "updated_at", "updatedat", "invoice_date", "invoicedate", "due_date", "duedate", "currency", "id", "reference_number", "tax_amount", "discount_amount", "amount_paid", "payment_method", "notes"];
          ...
          <div className="bg-white rounded-lg p-2.5 md:p-3 w-full md:w-[420px] max-w-[95vw] h-[75vh] md:h-auto md:max-h-[85vh] flex flex-col shadow-lg">
            ...
            <div className="flex-shrink-0 mt-2 pt-2 border-t">
              ...
              <div className="flex gap-1.5">
                <button
                  className="flex-1 px-2 py-1.5 rounded bg-gray-200 text-[10px] font-semibold"
